...
function renderParent(parent){
  ...
  return `<div class="page-heading"><div><p class="eyebrow">IDEA ${parent.number-1} · ${e(idea.domain)}</p><h1>${e(parent.title)}</h1><p class="intro">${e(idea.summary)}</p></div><span class="badge ${parent.id==='schools'?'success':'neutral'}">${parent.id==='schools'?'우선 검토':'아이디어 후보'}</span></div><div class="space-principle"><h2>${parent.id==='spaces'?'지역의 인구와 지형에 맞는 기존 공간':parent.id==='schools'?'입주 파동이 어린이집에서 고교까지 지나가는 시간표로 판단':'위험지역을 넘어, 끊어진 서비스 접근성을 진단'}</h2><p>${e(idea.need)}</p></div>${parent.id==='disaster'?disasterContext():parent.id==='schools'?schoolContext():''}<div class="section-bar"><h2>구체적인 계획</h2><span class="meta">각 계획은 주소가 있는 하위 페이지입니다.</span></div>${planCards(parent)}`;
}
function schoolContext(){
  const horizon=[['어린이집 영아반','0–2세 + 해당 학년도 출생','2027학년도 일부'],['유치원·어린이집 유아반','3–5세','2030학년도까지'],['초등학교','6–11세','2033학년도까지'],['중학교','12–14세','2039학년도까지'],['고등학교','15–17세','2042학년도까지']];
  ...
  return `<aside class="callout"><b>2026년까지 태어난 아이들로 결정되는 기간</b><p>학년도 T의 대상은 T−1년 12월 말 만 나이로 정합니다. 표의 기간 안에서는 출산보다 이동(아파트 입주)이 불확실성의 중심이고, 그 뒤에는 출산 시나리오가 더해집니다.</p><div class="table-wrap"><table><thead><tr><th>시설</th><th>대상 연령</th><th>이미 태어난 코호트로 결정</th></tr></thead><tbody>${horizon.map(row=>`<tr>${row.map(cell=>`<td>${e(cell)}</td>`).join('')}</tr>`).join('')}</tbody></table></div><details><summary>산단과 배후 주거의 발표 일정</summary><ul>${schedule.map(([date,text,url,label])=>`<li>${e(date)} · ${e(text)} · ${external(url,label)}</li>`).join('')}</ul><p class="meta">발표 목표이며 실제 입주·전입 관측치가 아닙니다. 첫 입주 목표가 2030년에서 2033년 하반기로 바뀐 폭은 지연 시나리오의 참고값입니다.</p></details></aside>`;
}
