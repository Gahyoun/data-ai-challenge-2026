...
function renderParent(parent){
  ...
  return `<div class="page-heading"><div><p class="eyebrow">IDEA ${parent.number-1} · ${e(idea.domain)}</p><h1>${e(parent.title)}</h1><p class="intro">${e(idea.summary)}</p></div><span class="badge ${parent.id==='disaster'?'success':'neutral'}">${parent.id==='disaster'?'우선 검토':'아이디어 후보'}</span></div><div class="space-principle"><h2>${parent.id==='spaces'?'지역의 인구와 지형에 맞는 기존 공간':parent.id==='schools'?'인구의 이동과 성장을 함께 보는 학교 계획':'위험지역을 넘어, 끊어진 서비스 접근성을 진단'}</h2><p>${e(idea.need)}</p></div>${parent.id==='disaster'?disasterContext():''}<div class="section-bar"><h2>구체적인 계획</h2><span class="meta">각 계획은 주소가 있는 하위 페이지입니다.</span></div>${planCards(parent)}`;
}
